<template>
	<div class="page-container m-3 d-flex flex-column">
		...
		<div class="guess-container d-flex flex-column">
			...
			<input name="guess" type="number" pattern="[0-9]" class="guess-input form-control" :min='gameData.minimum || 1' :max='gameData.maximum || 100' @keydown.enter="guess" v-model="guessedNumber">
		...
		<button class="quit-btn btn btn-danger mt-4" @click="$router.go(-1)">Quit & exit</button>
	</div>
...
export default {
	...
	methods: {
		guess(){
			...
			if(this.guessedNumber < this.gameData.minimum || this.guessedNumber > this.gameData.maximum) {
				this.playable = false;
